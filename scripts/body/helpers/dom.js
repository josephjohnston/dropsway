body.helpers.dom = {
	...
	makeSlider: function(parent,values,range,onchange) { // values: width,height,railColor,blockColor  range: [min,max,defaul]
		...
		rail.makeInactive = function() {
			...
			rail.set().background('gray')
			block.set().background('gray')
		}
		rail.makeActive = function() {
			inactive = false;
			rail.set().background(values.railColor);
			block.set().background(values.blockColor);
		}
		rail.hide = function() {
	...
	makeSelector: function(parent,left,top,buttonItems,buttonValues,popItems,popItemValues,callback,canvas) {
		var $ = {}, controls = body.build.controls, dom = this;
		$.onItem = false;
		// button
			...
			$.button.changeItem = function(itemIndex) {
				...
				if(canvas) {
					...
					this.currentItem.icon.regular(ctx,side);
				} else {
			...
			// canvas
				if(canvas) {
					...
					$.button.canvas.onmouseenter = function() {
						preservePointer();
						$.button.currentItem.icon.hover(ctx,side);
					}
					$.button.canvas.onmouseleave = function() {
						defaultCursor();
						$.button.currentItem.icon.regular(ctx,side);
					}
		...
		// popup
			...
			// scrolling
				...
				$.pop.div.onmousewheel = function(e) {
					...
					if(toBeTop+$.pop.points.padding<mouseY && mouseY<=toBeTop+$.pop.points.height-$.pop.points.padding
						&& toBeTop+$.pop.points.padding<$.button.points.top 
						&& $.button.points.top+/*controls.secHeight/1.5*/$.button.points.side<=toBeTop+$.pop.points.height-$.pop.points.padding) {
